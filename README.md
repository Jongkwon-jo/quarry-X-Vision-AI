...
| 형식 | 예시 | 시기 | 특징 |
|------|------|------|------|
| 현행 8자리 | `123가4567` | 2019.9~ | 단일 라인, 차종 3자리 |
| 구형 7자리 | `12가3456` | 2004~2019 | 단일 라인, 차종 2자리 |
| 지역명 2열 | 대구 / `06라5245` | 1996~2003 | 위 지역명 + 아래 본문 |

`AdaptiveSplitter`가 종횡비와 세로 프로필을 분석해 자동 판별 후 적절한 분리기로 라우팅합니다. 호출부는 형식 차이를 신경 쓰지 않아도 됩니다.

```python
from anpr_poc.pipeline import ANPRPipeline
pipeline = ANPRPipeline.from_default()
result = pipeline.run('plate.jpg')
print(result.plate_format.plate_format.value)  # 'plate_8' / 'plate_7' / 'plate_2row'
```

지역명 2열 번호판도 `EnhancedPlateValidator`가 본문을 추출해 차종 검증 수행. 등록된 지역 약칭 17개(서울·부산·대구·인천·광주·대전·울산·세종·경기·강원·충북·충남·전북·전남·경북·경남·제주) 외 텍스트는 SUSPECT로 분류됩니다.
